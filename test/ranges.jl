...
for T in (Float16, Float32, Float64)
    x = prevfloat(T(1))   # significand is all-1s
    hi, lo = Base.splitprec(x)
    @test hi + lo == x
    @test widen(hi)^2 == widen(hi^2)
    @test widen(hi)*widen(lo) == widen(hi*lo)
    @test 0 <= widen(lo)^2 - lo^2 <= 2*eps(T)
    for i = 1:10^3
        x = rand(T)
        hi, lo = Base.splitprec(x)
        @test hi+lo === x
...
function highprec_pair(x, y)
    ...
    # # Uncomment this if you're debugging multiplication
    # xh, xl = Base.splitprec(x)
    # yh, yl = Base.splitprec(y)
    # # exactness of hi-multiplication doesn't hold for subnormals
    # if !issubnormal(xh) && !issubnormal(yh)
    #     pw, p = widen(xh)*widen(yh), xh*yh
    #     if !isnan(p) && !issubnormal(p) && abs(pw) > subnormalmin(typeof(p))
    #         @test pw == widen(p)
    #     end
    # end
