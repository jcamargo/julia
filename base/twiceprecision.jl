...
## Veltkamp splitting
# Multiplication is based on fma, but splitting routines are still
# useful esp. for handling precise Integer->AbstractFloat conversions.

"""
    xh = halfprec(x)

Return a value close to `x` that has at most half of the precision
available to `typeof(x)`, removing `x`'s trailing bits. This is a
component of Veltkamp splitting, see "Handbook of Floating-point
Arithmetic", §4.4.1.

Note that this may overflow for large `x`. A safe strategy is
```julia
xs, xe = frexp(x)
xh = ldexp(halfprec(xs), xe)
```
"""
function halfprec(x::T) where {T<:AbstractFloat}
...
Return numbers `hi` and `lo` corresponding to the most and least
significant bits of `x`, respectively, such that `x = hi + lo`. Moreover, if

    hi1, lo1 = splitprec(x1)
    hi2, lo2 = splitprec(x2)

then pairwise products (e.g., `hi1 * hi2`, `lo1 * hi2`, etc.) can be
performed exactly (without loss of precision compared to widening,
unless one of the numbers is subnormal).

This is known as Veltkamp splitting, see "Handbook of Floating-point
Arithmetic", §4.4.1.
"""
...
Note that `lo` may have too many bits to be precise under
multiplication by other "split" floating point numbers---you may need
to call `splitprec(lo)` or use the Dekker arithmetic routines when
performing operations with `lo`.
"""
...
function truncmask(x::F, mask) where {F<:IEEEFloat}
    reinterpret(F, mask & reinterpret(fpinttype(F), x))
end
truncmask(x, mask) = x

function truncbits(x::F, nb) where {F<:IEEEFloat}
    truncmask(x, typemax(fpinttype(F)) << nb)
end
truncbits(x, nb) = x
